...
# 非虚拟化组件节点
默认情况下，自定义组件是“虚拟的”，即会展示自定义组件内部的第一层节点。但有些时候，开发者希望这个节点是一个“非虚拟化的”，使用时可以在这个节点设置 `id`、 `class`、 `style`，就如同一个基础组件。<br />这个时候，可以将该自定义组件设置为“非虚拟化”。支持版本基础库 2.8.0、IDE 3.2.3 及以上版本。
...
## :host 选择器
当开启 `virtualHost: false` 时，自定义组件可以使用 `:host` 选择器来指定该自定义组件的默认样式。支持版本基础库 2.8.0、IDE 3.2.3 及以上版本。
```css
:host {
  color: red;
}
